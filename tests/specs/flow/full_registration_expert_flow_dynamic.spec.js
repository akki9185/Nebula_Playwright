/**
 * MASTER REGISTRATION & LOGIN FLOW TEST (EXPERT PLAN - DYNAMIC CONFIGURATION)
 *
 * A single positive E2E test that performs exactly ONE registration from start to finish
 * with seats and search goals configured dynamically via environment variables:
 *  - FA: Total Full Access seats (e.g. FA=10, defaults to 5)
 *  - RO: Total Read-Only seats (e.g. RO=7, defaults to 7)
 *  - GOALS: Comma-separated list of search goals to select (e.g. GOALS=FC,PM, defaults to empty)
 */
...
test.describe('Master Expert Registration and Login Flow (Dynamic)', () => {
  test(
    ...
    async ({ page }) => {
      ...
      console.log(`Read-Only Seats Requested (RO)  : ${requestedRO}`);
      console.log(`Search Goals Requested (GOALS)  : ${JSON.stringify(requestedGoals)}`);
      console.log(`============================================================\n`);

      // Register response listener to debug any API errors
      page.on('response', async response => {
        if (response.url().includes('/api/') && response.status() >= 400) {
          try {
            console.error(`\n[API ERROR] ${response.request().method()} ${response.url()} status ${response.status()}`);
      ...
      // Adjust Full Access seats dynamically (Base plan starts with 5)
      const baseFA = 5;
      ...
      // Adjust Read-Only seats dynamically (Base plan starts with 0)
      if (requestedRO > 0) {
        console.log(`Adding ${requestedRO} Read-Only seats...`);
        for (let i = 0; i < requestedRO; i++) {
          await subPage.incrementReadOnly('Expert');
        }
      }

      // Select Search Goals dynamically
      for (const goal of requestedGoals) {
        console.log(`Selecting Search Goal: ${goal}`);
        await subPage.selectSearchGoal(goal);
      }
      ...
      await expect(userMgmtPage.sub_cellFullAccessSeats).toContainText(String(requestedFA));
      await expect(userMgmtPage.sub_cellFullAccessAvailable).toContainText(String(requestedFA - 1));
      await expect(userMgmtPage.sub_cellReadOnlySeats).toContainText(String(requestedRO));
      await expect(userMgmtPage.sub_cellReadOnlyAvailable).toContainText(String(requestedRO));
      ...
      // Poll subscription status with page reloads (to wait for async Stripe webhook database updates)
      console.log('── Waiting for subscription status to become paid/active (polling with page reloads) ──');
      let status = '';
      for (let i = 0; i < 15; i++) {
        status = await userMgmtPage.sub_cellStatus.innerText();
        if (!status.toLowerCase().includes('unpaid')) {
          console.log(`✓ Status updated to "${status}" on attempt ${i + 1}`);
          break;
        }
        console.log(`[Attempt ${i + 1}/15] Status is still "${status}". Reloading page in 3s...`);
        await page.waitForTimeout(3000);
        await page.reload();
        await userMgmtPage.tab_Subscription.waitFor({ state: 'visible', timeout: 15000 });
        await userMgmtPage.goToSubscriptionTab();
      }
      ...
      // ─────────────────────────────────────────────────────────────────────────
      // 8. Navigate to User Management & Invite members for all available seats
      // ─────────────────────────────────────────────────────────────────────────
      console.log('\n── Step 8: Navigating back to User Management and inviting members for all available seats ──');
      await page.goto('user-management');
      await userMgmtPage.goToSubscriptionTab();

      // Dynamically get available Full Access seats count
      const fullAccessAvailableText = await userMgmtPage.sub_cellFullAccessAvailable.innerText();
      const fullAccessAvailableCount = parseInt(fullAccessAvailableText.trim(), 10);
      console.log(`Initial available Full Access seats: ${fullAccessAvailableCount}`);
      ...
      for (let count = 1; count <= fullAccessAvailableCount; count++) {
        ...
        await expect(userMgmtPage.sub_cellFullAccessSeats).toContainText(String(requestedFA));
        await expect(userMgmtPage.sub_cellFullAccessAvailable).toContainText(String(expectedAvailableFullSeats));
        await expect(userMgmtPage.sub_cellReadOnlySeats).toContainText(String(requestedRO));
        await expect(userMgmtPage.sub_cellReadOnlyAvailable).toContainText(String(requestedRO));
        console.log(`✓ Verified Full Access available seats decreased to ${expectedAvailableFullSeats}`);
      }

      // Dynamically get available Read-Only seats count
      const readOnlyAvailableText = await userMgmtPage.sub_cellReadOnlyAvailable.innerText();
      const readOnlyAvailableCount = parseInt(readOnlyAvailableText.trim(), 10);
      console.log(`Initial available Read-Only seats: ${readOnlyAvailableCount}`);
      ...
      for (let count = 1; count <= readOnlyAvailableCount; count++) {
        ...
        await expect(userMgmtPage.sub_cellFullAccessSeats).toContainText(String(requestedFA));
        await expect(userMgmtPage.sub_cellFullAccessAvailable).toContainText('0');
        await expect(userMgmtPage.sub_cellReadOnlySeats).toContainText(String(requestedRO));
        await expect(userMgmtPage.sub_cellReadOnlyAvailable).toContainText(String(expectedAvailableReadOnlySeats));
        console.log(`✓ Verified Read Only available seats decreased to ${expectedAvailableReadOnlySeats}`);
      }
      ...
      await expect(fullAccessOption).toHaveAttribute('aria-disabled', 'true');
